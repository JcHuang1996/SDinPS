...
class MainProblemHatDataMethodName:
...
class MainProblemModelTypeName:
    ...
    COLLAPSED_BY_SCENARIOS = 'collapsed_by_scenarios'
...
class InputDataName:
    ...
    DG_RATED_POWER_DF = 'dg_rated_power_df'                    # dataframe, DG type and rated power
...
class DataName:
    ...
    DICT_HAT_DEMAND_ACTIVE = 'dict_hat_demand_active'   # dict, {j: float, ...}, collapsed active demand for main problem
    ...
    DICT_HAT_DEMAND_ACTIVE_BY_SCENARIO = 'dict_hat_demand_active_by_scenario'  # dict, {(j,s): float, ...}, time-collapsed active demand by scenario for main problem
    DICT_HAT_DEMAND_REACTIVE_BY_SCENARIO = 'dict_hat_demand_reactive_by_scenario'  # dict, {(j,s): float, ...}, time-collapsed reactive demand by scenario for main problem
    DICT_HAT_LINE_HEALTHY_NH_BY_SCENARIO = 'dict_hat_line_healthy_n_harden_by_scenario'  # dict, {(i,j,s): status_binary, ...}, time-collapsed non-hardened line health by scenario for main problem
    DICT_HAT_LINE_HEALTHY_H_BY_SCENARIO = 'dict_hat_line_healthy_harden_by_scenario'  # dict, {(i,j,s): status_binary, ...}, time-collapsed hardened line health by scenario for main problem
    ...
    DICT_DG_RATED_POWER = 'dict_dg_rated_power'         # dict, {dg_type: rated_power, ...}, from dg_rated_power.csv
    DICT_DG_UNIT_PRICE = 'dict_dg_unit_price'           # dict, {dg_type: unit_price, ...}, from dg_rated_power.csv
    DICT_DG_EXTRA_ADJUSTMENT = 'dict_dg_extra_adjustment'   # dict, {dg_type: extra_adjustment, ...}, from dg_rated_power.csv
...
class VarName:
    ...
    DG_INSTALL_TYPE = 'xg_type'         # Binary, xg_type_j,typ indicating whether DG of type typ is installed at node j. Key (j, typ)
    ...
    # === Added master-only collapsed SP variables ===
    MAIN_LINE_CONNECTED = 'm_yc'                    # Binary, collapsed line connectivity for main problem
    MAIN_LOAD_SHED_RATIO = 'm_yl'                   # Continuous, collapsed load shed ratio for main problem
    MAIN_BUS_VOLTAGE = 'm_v'                        # Continuous, collapsed bus voltage for main problem
    MAIN_DG_ACTIVE_POWER = 'm_pg'                   # Continuous, collapsed active DG generation for main problem
    MAIN_DG_REACTIVE_POWER = 'm_qg'                 # Continuous, collapsed reactive DG generation for main problem
    MAIN_LINE_ACTIVE_FLOW = 'm_p'                   # Continuous, collapsed active line flow for main problem
    MAIN_LINE_REACTIVE_FLOW = 'm_q'                 # Continuous, collapsed reactive line flow for main problem
    MAIN_VIRTUAL_LINE_FLOW = 'm_ul'                 # Continuous, collapsed virtual line flow for main problem
    MAIN_VIRTUAL_SOURCE_INDICATOR = 'm_us'          # Binary, collapsed virtual source indicator for main problem
    MAIN_VIRTUAL_INJECT_POWER = 'm_ui'              # Continuous, collapsed virtual injected power for main problem
...
class ConstrName:
    ...
    # DG_INSTALL_TYPE_UB = 'dg_install_type_ub'           # DG_INSTALL_TYPE[j,typ] <= DG_INSTALL[j]
    DG_INSTALL_TYPE_ONE = 'dg_install_type_one'         # exactly one type per node when installed
    DG_OPERATION = 'dg_op'                              # AFN: Generator operation (DG_RATED_POWER linked to DG_INSTALL_TYPE)
    OPTIMAL_CUT = 'opt_cut'                             # AFN: Optimal Cut (generated by solving sub-problems)

    MAIN_DG_OPERATION = 'main_dg_op'                    # Collapsed DG rated power linking in main problem
    MAIN_LINE_CONNECTED = 'main_line_conn'              # Collapsed line connected condition in main problem
    MAIN_VIRTUAL_FLOW_BAL_C1 = 'main_virt_flow_c1'      # Collapsed virtual power flow balance (eq. 1)
    MAIN_VIRTUAL_FLOW_BAL_C2 = 'main_virt_flow_c2'      # Collapsed virtual power flow balance (eq. 2)
    MAIN_NO_VIRTUAL_ON_OPEN = 'main_virt_no_open'       # Collapsed no virtual flow on open line
    MAIN_RADIALITY = 'main_radiality'                   # Collapsed radiality
    MAIN_DG_ACTIVE_POWER_UB = 'main_dg_p'               # Collapsed DG active power upper bound
    MAIN_DG_REACTIVE_POWER_UB = 'main_dg_q'             # Collapsed DG reactive power upper bound
    MAIN_VOLTAGE_RANGE_C1 = 'main_volt_rng_c1'          # Collapsed voltage range lower bound
    MAIN_VOLTAGE_RANGE_C2 = 'main_volt_rng_c2'          # Collapsed voltage range upper bound
    MAIN_VOLTAGE_SLACK_BUS = 'main_slack_bus'           # Collapsed slack bus voltage equality
    MAIN_VOLTAGE_FLOW_REL = 'main_volt_flow'            # Collapsed voltage-flow relationship
    MAIN_FLOW_BALANCE_C1 = 'main_flow_bal_p'            # Collapsed active power balance
    MAIN_FLOW_BALANCE_C2 = 'main_flow_bal_q'            # Collapsed reactive power balance
    MAIN_NO_FLOW_ON_OPEN_C1 = 'main_no_flow_open_p'     # Collapsed active no-flow-on-open-line
    MAIN_NO_FLOW_ON_OPEN_C2 = 'main_no_flow_open_q'     # Collapsed reactive no-flow-on-open-line
    MAIN_LINE_THERMAL_C1 = 'main_thermal_c1'            # Collapsed thermal limitation (part 1)
    MAIN_LINE_THERMAL_C2 = 'main_thermal_c2'            # Collapsed thermal limitation (part 2)
    MAIN_LINE_THERMAL_C3 = 'main_thermal_c3'            # Collapsed thermal limitation (part 3)
...
class ObjName:
    ...
    MAIN_APPROX_DG_GENERATING_COST = 'main_dg_generating_cost'
    MAIN_APPROX_LOAD_SHED_COST = 'main_load_shed_cost'
